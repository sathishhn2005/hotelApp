...
BEGIN
    ...
    select PlaceOrderId,PO.FoodProductId,FoodName,PO.CompanyID,PO.CustomerID,PO.CustomerName,PO.PhoneNumber,PO.TotalAmount,PO.Quantity,PO.Tax,PO.TableNo,PO.RazorOrderDetailsId
,PO.RazorPaymentId,PaymentStatus,PaymentType,PO.Comments
...
,PO.RazorPaymentId,PaymentStatus,PaymentType,PO.Comments,'pending' as Status
 From tb_placeorder PO
Inner join tb_foodproducts FP ON FP.FoodProductId=PO.FoodProductId and FP.CompanyID=PO.CompanyID
...
set IsExists=(Select count(*) from TB_Admin WHERE CompanyName=CompName and username=Uname);
if IsExists=0 then
...
set _ReturnValue=0;
select CompanyId,CompanyName,AdminName,PhoneNumber,Address,UserName,Password,AccRegisteredDate,Cost,Status from tb_admin where CompanyId=_ReturnValue;
end if;
end$$
DELIMITER ;
...
Select OrderDetailsId,FoodProductId,CompanyID,CustomerID,TotalAmount,Discount,UnitPrice,Quantity,Tax,TableNo
...
Insert into TB_PlaceOrder (FoodProductId,CompanyID,CustomerID,CustomerName,PhoneNumber,TotalAmount,Quantity,Tax,TableNo,PaymentType,RazorOrderDetailsId,Comments)
select FoodProdid,CompID,CustID,CustName,PhoneNo,TotalAmt,Qty,TaxPer,TableId,PaymentType,OrderId,Cmnt; 
set _ReturnValue=(select last_insert_id());
if _ReturnValue>0 then
...
RazorOrderDetailsId,Comments from TB_PlaceOrder where PlaceOrderId=_ReturnValue;
end if;
END$$
DELIMITER ;
...
BEGIN
declare CId bigint;
set CId=(Select distinct CompanyId from tb_admin where UserName=UserN and Password=Pswd);
if(Cid>0) then
...
BEGIN
UPDATE TB_ADMIN SET Status =PauseStatus WHERE CompanyId= CompnId;
END$$
DELIMITER ;
